...
def main():
    ...
    try:
        _, grid_lat, grid_lon = extract_value(
            file_list[0], VARIABLE_NAME, TARGET_LAT, TARGET_LON
        )
    ...
    logger.info(
        f"TASK: Create time series plot for variable '{VARIABLE_NAME}' at ({TARGET_LAT:.2f}°, {TARGET_LON:.2f}°)"
    )
    ...
    for file_path in file_list:
        ...
        if timestamp is None:
            continue
        try:
            value, _, _ = extract_value(
                file_path, VARIABLE_NAME, TARGET_LAT, TARGET_LON
            )
            values.append(value)
    ...
    mode_label = (
        "Nowcast"
        if FORECAST_PREVIEW_TIME is None
        else f"Forecast {FORECAST_PREVIEW_TIME}"
    )
    plt.figure(figsize=(10, 5))
    plt.plot(times, values, marker="o")
    plt.title(
        f"{VARIABLE_NAME.upper()} ({mode_label}) at ({grid_lat:.2f}°, {grid_lon:.2f}°)"
    )
    plt.xlabel("Time")
