...
if __name__ == "__main__":
    ...
    for i in range(0, num_samples):
        ...
        time_index_6 = np.searchsorted(
            time_var[:], nc.date2num(time - timedelta(hours=6), units=time_var.units)
        )
        time_index_12 = np.searchsorted(
            time_var[:], nc.date2num(time - timedelta(hours=12), units=time_var.units)
        )
        time_index_18 = np.searchsorted(
            time_var[:], nc.date2num(time - timedelta(hours=18), units=time_var.units)
        )
        lat_idx = np.abs(lat_var[:] - lats[i]).argmin()
        lon_idx = np.abs(lon_var[:] - lons[i]).argmin()
        wave_heights.append(
            ocean_data.variables["sig_wave_height"][time_index, lat_idx, lon_idx]
        )
        wave_heights_6.append(
            ocean_data.variables["sig_wave_height"][time_index_6, lat_idx, lon_idx]
        )
        wave_heights_12.append(
            ocean_data.variables["sig_wave_height"][time_index_12, lat_idx, lon_idx]
        )
        wave_heights_18.append(
            ocean_data.variables["sig_wave_height"][time_index_18, lat_idx, lon_idx]
        )
        wave_periods.append(
            ocean_data.variables["wave_period"][time_index, lat_idx, lon_idx]
        )
        wave_periods_6.append(
            ocean_data.variables["wave_period"][time_index_6, lat_idx, lon_idx]
        )
        wave_periods_12.append(
            ocean_data.variables["wave_period"][time_index_12, lat_idx, lon_idx]
        )
        wave_periods_18.append(
            ocean_data.variables["wave_period"][time_index_18, lat_idx, lon_idx]
        )
        wave_directions.append(
            ocean_data.variables["wave_direction"][time_index, lat_idx, lon_idx]
        )
        wave_directions_6.append(
            ocean_data.variables["wave_direction"][time_index_6, lat_idx, lon_idx]
        )
        wave_directions_12.append(
            ocean_data.variables["wave_direction"][time_index_12, lat_idx, lon_idx]
        )
        wave_directions_18.append(
            ocean_data.variables["wave_direction"][time_index_18, lat_idx, lon_idx]
        )
        wind_directions.append(
            ocean_data.variables["wind_direction"][time_index, lat_idx, lon_idx]
        )
        wind_directions_6.append(
            ocean_data.variables["wind_direction"][time_index_6, lat_idx, lon_idx]
        )
        wind_directions_12.append(
            ocean_data.variables["wind_direction"][time_index_12, lat_idx, lon_idx]
        )
        wind_directions_18.append(
            ocean_data.variables["wind_direction"][time_index_18, lat_idx, lon_idx]
        )
        wind_speeds_6.append(
            ocean_data.variables["wind_speed"][time_index_6, lat_idx, lon_idx]
        )
        wind_speeds_12.append(
            ocean_data.variables["wind_speed"][time_index_12, lat_idx, lon_idx]
        )
        wind_speeds_18.append(
            ocean_data.variables["wind_speed"][time_index_18, lat_idx, lon_idx]
        )
        wind_speeds.append(
            ocean_data.variables["wind_speed"][time_index, lat_idx, lon_idx]
        )
        time_index = np.searchsorted(
            time_var[:],
            nc.date2num(time + timedelta(hours=time_amount[i]), units=time_var.units),
        )
        wave_height_correct.append(
            ocean_data.variables["sig_wave_height"][time_index, lat_idx, lon_idx]
        )
        wave_heading_correct.append(
            ocean_data.variables["wave_direction"][time_index, lat_idx, lon_idx]
        )
        wave_period_correct.append(
            ocean_data.variables["wave_period"][time_index, lat_idx, lon_idx]
        )
        time_index_48 = np.searchsorted(
            time_var[:], nc.date2num(time + timedelta(hours=48), units=time_var.units)
        )
    ...
    training_data_input = torch.tensor(
        np.column_stack(
            (
                wave_heights[0 : (int)(num_samples * 0.7)],
                wave_heights_18[0 : (int)(num_samples * 0.7)],
                wave_heights_12[0 : (int)(num_samples * 0.7)],
                wave_heights_6[0 : (int)(num_samples * 0.7)],
                wave_periods[0 : (int)(num_samples * 0.7)],
                wave_periods_18[0 : (int)(num_samples * 0.7)],
                wave_periods_12[0 : (int)(num_samples * 0.7)],
                wave_periods_6[0 : (int)(num_samples * 0.7)],
                wave_directions[0 : (int)(num_samples * 0.7)],
                wave_directions_18[0 : (int)(num_samples * 0.7)],
                wave_directions_12[0 : (int)(num_samples * 0.7)],
                wave_directions_6[0 : (int)(num_samples * 0.7)],
                wind_directions[0 : (int)(num_samples * 0.7)],
                wind_directions_18[0 : (int)(num_samples * 0.7)],
                wind_directions_12[0 : (int)(num_samples * 0.7)],
                wind_directions_6[0 : (int)(num_samples * 0.7)],
                wind_speeds[0 : (int)(num_samples * 0.7)],
                wind_speeds_18[0 : (int)(num_samples * 0.7)],
                wind_speeds_12[0 : (int)(num_samples * 0.7)],
                wind_speeds_6[0 : (int)(num_samples * 0.7)],
                time_amount[0 : (int)(num_samples * 0.7)],
            )
        ),
        dtype=torch.float32,
    )

    training_data_correct_out_real = torch.tensor(
        np.column_stack(
            (
                wave_height_correct[0 : (int)(num_samples * 0.7)],
                wave_heading_sin[0 : (int)(num_samples * 0.7)],
                wave_heading_cos[0 : (int)(num_samples * 0.7)],
                wave_period_correct[0 : (int)(num_samples * 0.7)],
            )
        ),
        dtype=torch.float32,
    )
    # training_data_correct_out_real=torch.tensor((wave_height_correct[0:(int)(num_samples*0.7)]), dtype=torch.float32).view(-1,1)
    validation_data_input = torch.tensor(
        np.column_stack(
            (
                wave_heights[(int)(num_samples * 0.7) : (int)(num_samples * 0.9)],
                wave_heights_18[(int)(num_samples * 0.7) : (int)(num_samples * 0.9)],
                wave_heights_12[(int)(num_samples * 0.7) : (int)(num_samples * 0.9)],
                wave_heights_6[(int)(num_samples * 0.7) : (int)(num_samples * 0.9)],
                wave_periods[(int)(num_samples * 0.7) : (int)(num_samples * 0.9)],
                wave_periods_18[(int)(num_samples * 0.7) : (int)(num_samples * 0.9)],
                wave_periods_12[(int)(num_samples * 0.7) : (int)(num_samples * 0.9)],
                wave_periods_6[(int)(num_samples * 0.7) : (int)(num_samples * 0.9)],
                wave_directions[(int)(num_samples * 0.7) : (int)(num_samples * 0.9)],
                wave_directions_18[(int)(num_samples * 0.7) : (int)(num_samples * 0.9)],
                wave_directions_12[(int)(num_samples * 0.7) : (int)(num_samples * 0.9)],
                wave_directions_6[(int)(num_samples * 0.7) : (int)(num_samples * 0.9)],
                wind_directions[(int)(num_samples * 0.7) : (int)(num_samples * 0.9)],
                wind_directions_18[(int)(num_samples * 0.7) : (int)(num_samples * 0.9)],
                wind_directions_12[(int)(num_samples * 0.7) : (int)(num_samples * 0.9)],
                wind_directions_6[(int)(num_samples * 0.7) : (int)(num_samples * 0.9)],
                wind_speeds[(int)(num_samples * 0.7) : (int)(num_samples * 0.9)],
                wind_speeds_18[(int)(num_samples * 0.7) : (int)(num_samples * 0.9)],
                wind_speeds_12[(int)(num_samples * 0.7) : (int)(num_samples * 0.9)],
                wind_speeds_6[(int)(num_samples * 0.7) : (int)(num_samples * 0.9)],
                time_amount[(int)(num_samples * 0.7) : (int)(num_samples * 0.9)],
            )
        ),
        dtype=torch.float32,
    )

    validation_data_correct_out_real = torch.tensor(
        np.column_stack(
            (
                wave_height_correct[
                    (int)(num_samples * 0.7) : (int)(num_samples * 0.9)
                ],
                wave_heading_sin[(int)(num_samples * 0.7) : (int)(num_samples * 0.9)],
                wave_heading_cos[(int)(num_samples * 0.7) : (int)(num_samples * 0.9)],
                wave_period_correct[
                    (int)(num_samples * 0.7) : (int)(num_samples * 0.9)
                ],
            )
    ...
    for epoch in range(num_epochs):
        ...
        loss = weighted_mse_loss(
            predictions, training_data_correct_out_real
    ...
    val_time_amounts = (
        val_time_amounts * train_std[0, -1].item() + train_mean[0, -1].item()
    )
    ...
    plt.plot(
        [min(val_dataplot_height), max(val_dataplot_height)],
        [min(val_dataplot_height), max(val_dataplot_height)],
        "r--",
        label="Ideal Fit (y=x)",
    )
    ...
    plt.figure(figsize=(8, 6))
    bins = np.linspace(
        min(min(val_dataplot_height), min(val_predplot_height)),
        max(max(val_dataplot_height), max(val_predplot_height)),
        31,
    )
    plt.hist(
        val_dataplot_height, bins=bins, alpha=0.5, label="Actual", edgecolor="black"
    )
    plt.hist(
        val_predplot_height, bins=bins, alpha=0.5, label="Predicted", edgecolor="black"
    )
    ...
    errors = np.abs(
        np.rad2deg(
            np.arctan2(
                np.sin(predicted_radians - actual_radians),
    ...
    plt.figure(figsize=(8, 6))
    bins = np.linspace(
        min(min(val_dataplot_heading), min(val_predplot_heading)),
        max(max(val_dataplot_heading), max(val_predplot_heading)),
        31,
    )
    plt.hist(
        val_dataplot_heading, bins=bins, alpha=0.5, label="Actual", edgecolor="black"
    )
    plt.hist(
        val_predplot_heading, bins=bins, alpha=0.5, label="Predicted", edgecolor="black"
    )
    ...
    plt.colorbar(sc, label="Time Interval (hours)")
    plt.plot(
        [min(val_dataplot_period), max(val_dataplot_period)],
        [min(val_dataplot_period), max(val_dataplot_period)],
        "r--",
        label="Ideal Fit (y=x)",
    )
    ...
    plt.figure(figsize=(8, 6))
    print(
        "val_dataplot_period min/max:",
        min(val_dataplot_period),
        max(val_dataplot_period),
    )
    print(
        "val_predplot_period min/max:",
        min(val_predplot_period),
        max(val_predplot_period),
    )
    bins = np.linspace(
        min(min(val_dataplot_period), min(val_predplot_period)),
        max(max(val_dataplot_period), max(val_predplot_period)),
        31,
    )
    print("bins:", bins)
    plt.hist(
        val_dataplot_period, bins=bins, alpha=0.5, label="Actual", edgecolor="black"
    )
    plt.hist(
        val_predplot_period, bins=bins, alpha=0.5, label="Predicted", edgecolor="black"
    )
