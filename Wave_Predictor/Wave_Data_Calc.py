...
from Ocean.weather_data_interpolation_API import planning_interpolation, load_netcdf_folder
class Wave_Data_Calc():
 ...
 def __init__(self,weather_input_dir,fc_input_dir=None, model_path=None):
  ...
  self.weather_data, issue_dts, fcst_dts = load_netcdf_folder(self.weather_input_dir)
 
  if self.use_fc:
    self.forecast_data, issue_dts, fcst_dts = load_netcdf_folder(self.fc_input_dir)
 ...
 def calc_WaveData(self,waypoint,current_time,future_time):
         ...
         if(self.use_fc and time_amount>=0):
          key = (round(waypoint.lat,1), round(waypoint.long,1), 
               self.round_datetime_to_nearest_hour(current_time), self.round_datetime_to_nearest_hour(future_time))
          if key in self.forecast_cache:
            result=self.forecast_cache[key]
            return result
          result=self.get_forecast_data(waypoint.lat,waypoint.long,current_time,time_amount)
          ...
          if key in self.model_cache:
            ...
            return result
          model_data_input=self.extract_model_features(waypoint.lat,waypoint.long,current_time,time_amount)
          ...
          self.model_cache[key] = result
           ...
           result=( results.get("swh")[0],results.get("dirpw")[0],results.get("perpw")[0])
 ...
 def extract_model_features(self, lat, lon, current_time, time_amount):
    ...
    model_data_input = torch.tensor(np.stack([
        d0["swh"],  d18["swh"],  d12["swh"],  d6["swh"],
        d0["perpw"], d18["perpw"], d12["perpw"], d6["perpw"],
        d0["dirpw"],  d18["dirpw"],  d12["dirpw"],  d6["dirpw"],
        d0["wdir"], d18["wdir"], d12["wdir"], d6["wdir"],
        d0["ws"], d18["ws"], d12["ws"], d6["ws"],
        time_amount
    ]), dtype=torch.float32)
