...
class propulsionSimulationBase(AbstractMachineryModel):
    ...
    def runMachinery(self, speed, vessel, time):
        ...
        resistance_in_kn, wake_fraction, thrust_deduction = self.resistance_model.estimateResistance(speed, vessel)
        eta_O, P_deliv, RPM = self.propulsion_model.estimatePropulsion(speed, resistance_in_kn, wake_fraction, thrust_deduction, vessel,self.resistance_model.rho)
        ...
        return fuel_weight,sfc,(eta_O,P_deliv,RPM),(resistance_in_kn,wake_fraction,thrust_deduction)
...
class propulsionModelBase:
    ...
    def estimatePropulsion(self, speed, resistance_in_kn, wake_fraction, thrust_deduction, vessel,rho):
        '''
...
class NPL_ResistanceModel(resistanceModelBase):
    ...
    def __init__(self, rho, rho_air, nu_seawater, C_A, res_file="Molland_NPL_Data_NumpyRead.csv"):
    ...
    def _checkBounds(self, BT, L_vol, VkLf):
        ...
        if VkLf > 4.1:
            VkLf = 4.1
            status = False
            logger.warning("Speed is above the regression range for resistance estimation - set to upper bound of 4.1")
        ...
        if VkLf < 3.2:
            if L_vol < 4.47:
                status = False
                L_vol = 4.47
                logger.warning("L/Vol is below the regression range for resistance estimation - set to lower bound of 4.47")
            if L_vol > 8.3:
                status = False
                L_vol = 8.3
                logger.warning("L/Vol is above the regression range for resistance estimation - set to upper bound of 8.3")    
            BT_lower_lim = np.interp(L_vol, [4.47,4.86,5.23,5.76, 6.59, 7.1, 8.3], [1.72,2.19,1.95,1.93,2.01,2.51,4.02])
            BT_upper_lim = np.interp(L_vol, [4.47,4.86,5.23,5.76, 6.59, 7.1, 8.3], [3.19,4.08,5.10,6.8,5.49,6.87,5.80])
            if BT < BT_lower_lim:
                status = False
                BT = BT_lower_lim
                logger.warning("B/T is below the regression range for resistance estimation - set to lower bound " + str(BT_lower_lim))
            if BT > BT_upper_lim:
                status = False
                BT = BT_upper_lim
                logger.warning("B/T is above the regression range for resistance estimation - set to upper bound" + str(BT_upper_lim))
        else:  #At higher speeds, only the lighter hulls were tested
            if L_vol < 4.86:
                status = False
                L_vol = 4.86
                logger.warning("L/Vol is below the regression range for resistance estimation - set to lower bound of 4.86")
            if L_vol > 8.3:
                status = False
                L_vol = 8.3
                logger.warning("L/Vol is above the regression range for resistance estimation - set to upper bound of 8.3")    
            BT_lower_lim = np.interp(L_vol, [4.86,5.23,5.76, 6.59, 7.1, 8.3], [2.19,1.95,1.93,2.01,2.51,4.02])
            BT_upper_lim = np.interp(L_vol, [4.86,5.23,5.76, 6.59, 7.1, 8.3], [4.08,5.10,6.8,5.49,6.87,5.80])
            if BT < BT_lower_lim:
                BT = BT_lower_lim
                status = False
                logger.warning("B/T is below the regression range for resistance estimation - set to lower bound " + str(BT_lower_lim))
    ...
    def estimateResistance(self, speed, vessel):
        ...
        try:
            ...
            logger.error("Vessel does not have hydrostatic properties needed for resistance estimation")
        ...
        Cr = spline(VkLf)/1000.  #Raw data was x1000 in textbook
        ...
        #Do air drag
        R_air = C_Air * 0.5 * Windage_Area *self.rho_air * VMS**2.0/1000.  #in Kn
    ...
    if(self.J[i]!=0 and self.J[i-1]!=0):   
     ...
     if(T1<T2):
       if(T2>=goal_treq and T1<=goal_treq): 
        ...
        final_J=self.J[i-1]+((x-(i-1))*(self.J[i]-self.J[i-1]))
        eta_O=self.eta[i-1]+((x-(i-1))*(self.eta[i]-self.eta[i-1]))
        break
     else:
       if(T2<=goal_treq and T1>=goal_treq):
...
class RPM_Power_EngineModel:
    def __init__(self, RPM, power, fuel, gear_ratio, shaft_eff=0.97):
        ...
        self.RPM_flat, self.power_flat, self.fuel_flat = self.flatten_data(RPM, power, fuel)
    ...
    def getSFC(self, RPM, P_deliv, vessel=None):
       ...
       '''
        brake_power = P_deliv / self.shaft_eff
        cs_power=CubicSpline(self.RPM,self.power)
        rbf_fuel = Rbf(self.RPM_flat, self.power_flat, self.fuel_flat, function='linear')
        ...
        if brake_power > np.max(cs_power(RPM)):
            logger.error("The requested power is beyond what the engine can produce. Returning SFC at max power")
            brake_power=np.max(cs_power(RPM))
        
        
        return float(rbf_fuel(RPM, brake_power))  # Convert the result to a single scalar value
...
def Engine_model_test(vessel):
    ...
    print(f"RPM Engine Model, RPM=100(power too large): {engine1.getSFC(100, 92, vessel)}")
    engine2 = RPM_Power_EngineModel(RPM2, power2, fuel2, 10)
    print(f"RPM Power Engine Model, RPM=100,Power 96 kW: {engine2.getSFC(100, 96, vessel)}")
    print(f"RPM Power Engine Model, RPM=170, Power 500 kW: {engine2.getSFC(170, 500, vessel)}") 
    print(f"RPM Power Engine Model, RPM=170, Power 700(too big)  kW: {engine2.getSFC(170, 700, vessel)}")                       
    print(f"RPM Power Engine Model, RPM=7(too small), Power 50 kW: {engine2.getSFC(7, 50, vessel)}")
    print(f"RPM Power Engine Model, RPM=7(too small), Power 500kW(too big) kW: {engine2.getSFC(7, 5000, vessel)}")

if __name__ == "__main__":
    #Create a resistance model using the NPL Series
    res_model = NPL_ResistanceModel(1025.0, 1.225, 1.1395e-6, 0.0, "/home/remecca/core/Simulation/Molland_NPL_Data_NumpyRead.csv")
    ...
    eta,delivered_power,RPM=propeller.estimatePropulsion(25.0,Rt,wt,t,vessel,res_model.rho)
    print(f"open water efficiency at 25 kts: {eta}")
    print(f"delivered power at 25 kts: {delivered_power}")
    print(f"RPM at 25 kts: {RPM}")
    eta,delivered_power,RPM=propeller.estimatePropulsion(15.0,Rt,wt,t,vessel,res_model.rho)
    print(f"open water efficiency at 15 kts: {eta}")
    print(f"delivered power at 15 kts: {delivered_power}")
    print(f"RPM at 15 kts: {RPM}")
    eta,delivered_power,RPM=propeller.estimatePropulsion(10.0,Rt,wt,t,vessel,res_model.rho)
