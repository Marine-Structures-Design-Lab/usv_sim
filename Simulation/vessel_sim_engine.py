...
weath_input_dir_lat40_lon_neg_140=script_dir.parent / 'Ocean/weather_2_nc/1201_1231_lat30_lon30_fc0_ts6'
fc_input_dir_lat40_lon_neg_140=script_dir.parent / 'Ocean/weather_2_nc/1215_1225_lat10_lon10_fc384_ts9'
weath_input_dir_lat35_lon155=script_dir.parent / 'Ocean/weather_2_nc/1210_1231_lat30_40_lon150_160_fc0_ts6'
fc_input_dir_lat35_lon155=script_dir.parent / 'Ocean/weather_2_nc/1218_1228_lat30_40_lon150_160_fc350_ts9'
...
class Destination:
   ...
   def __init__(self, waypoint, arrival_time, priority):
      ...
      self.estimated_arrival_time=arrival_time #set based on GA mission planning prediction
      self.priority=priority
      self.visited=False
      self.id=0 #db id
      self.time_blacklist=[] #list of datetime objects, holds times when the weather is dangerous around this waypoint
...
class Mission:
    ...
    def __init__(self, destinations,speeds=[]):
        ...
        self.current_waypoint_index = 1   #the current destination the vessel is traveling to
...
class vesselBase:
    ...
    '''
    def __init__(self,name,sim_name,start,end,mission,weather_input_dir,fc_input_dir=None,model_path=None,speed=20,heading=0,fuel_weight=100,speed_options=[]):
        '''
        Summary
        -----------------------
        ...
        self.wave_calc=Wave_Data_Calc(weather_input_dir,fc_input_dir,model_path) #Wave_Data_Calc object for calculating wave height, heading, and period
        self.opt_mission=None
        self.position=Waypoint(start.waypoint.lat,start.waypoint.long) #vessel's current position
        self.methods = []
        self.active=True
        self.replan_count=0
        self.vessel_id = db_manager.insert_vessel(self.name, sim_name,'cargo', 100, 'good', 100)
        self.mission_id = db_manager.insert_mission(self.vessel_id,self.full_mission.total_priority,f"{self.name}'s mission", start.arrival_time)
        self.ml_preds=[] # list of all (height, heading, period) tuples predicted by the simulation)
        self.actual_values=[] # list of all (height, heading, period) 
        #self.count=0
        for dest in self.full_mission.destinations:
            dest.id = db_manager.insert_waypoint(dest.waypoint.lat, dest.waypoint.long, dest.priority)
    ...
    def navigate(self, current_time, time_interval):
        ...
        '''
        destination = Waypoint(self.opt_mission.current_waypoint().lat,self.opt_mission.current_waypoint().long)
        goal_time=self.opt_mission.current_goal_time()
        estimated_time=self.opt_mission.current_predicted_arrival_time()
        g = geod.Inverse(self.position.lat, self.position.long, destination.lat, destination.long)
        self.heading = g['azi1']
        distance = g['s12'] / 1852
        wave_height, wave_heading,wave_period = self.wave_calc.calc_WaveData(self.position,current_time,current_time)
        ...
        if(speed==-1):
         self.opt_mission.destinations[self.opt_mission.current_waypoint_index].time_blacklist.append(current_time)
         self.opt_mission.destinations[self.opt_mission.current_waypoint_index].time_blacklist.append(estimated_time)
         problem_dest=self.opt_mission.destinations[self.opt_mission.current_waypoint_index]
         for dest in self.opt_mission.destinations:
            if dest!=problem_dest:
               g = geod.Inverse(problem_dest.waypoint.lat, problem_dest.waypoint.long, dest.waypoint.lat, dest.waypoint.long)
               distance = g['s12'] / 1852
               if distance<10:
         ...
         self.add_opt_mission_to_db(f" Detected danger at ({self.position.lat:.2f}, {self.position.long:.2f}) on {check_time}\n",check_time)
       ...
       # Time elapsed in the current interval
        ...
        while remaining_time_interval > 0:
            ...
            if self.is_at_waypoint(destination, remaining_time_interval):
                distance_to_waypoint = g['s12'] / 1852  # Convert meters to nautical miles
                ...
                logger.info(f"{self.name} reached waypoint at ({destination.lat}, {destination.long}) at {current_time} with a fuel weight of {self.fuel_weight} kN\n")
                self.calc_time_diff(current_time, goal_time,True)
                time_diff=self.calc_time_diff(current_time, estimated_time)
                if(time_diff<-10):
                   logger.info(f"{self.name}'s mission is {abs(time_diff)} hours behind predicted schedule, planning new mission")
                   self.plan_Optimal_Mission(current_time)
                   
                   self.add_opt_mission_to_db(f"{abs(time_diff)} hours behind predicted schedule",current_time)
                   log_entry = ("\n===========================\n" f"[{current_time}] Replanning Triggered for {self.name}\n"
                 "===========================\n\n" "Reason:\n"
                ...
                self.position =Waypoint(destination.lat,destination.long) #set current position to waypoint
                self.opt_mission.destinations[self.opt_mission.current_waypoint_index ].estimated_arrival_time=current_time
                ...
                pos=self.calculate_new_position(self.position,self.speed,remaining_time_interval)
                self.position.lat=pos[0]
                self.position.long=pos[1]
                self.timestep(remaining_time_interval,current_time)
                remaining_time_interval = 0
        #update log datatable
        db_manager.insert_log(self.vessel_id, self.mission_id, current_time.strftime('%Y-%m-%d %H:%M:%S'), self.position.lat, self.position.long, self.speed, 'Good', self.fuel_weight, float(wave_height), float(wave_period))
        logger.info(f"{current_time+timedelta(hours=time_interval)}: {self.name} at ({self.position.lat},{self.position.long}) with speed {self.speed} knots \n")#and fuel weight {self.fuel_weight}")
        return True
    ...
    def adjust_speed(self,startpoint,distance,start_time,current_time):
        ...
        # Get predicted wave data
         ...
         wave_height, wave_heading, wave_period = self.wave_calc.calc_WaveData(position, start_time, current_time)
         #self.count+=1
         '''
         ...
         if wave_model_key not in self.rao_lib.model_dict:
            model = Bretschneider("wave spectrum", wave_model_key[0], wave_model_key[1],0.01, 2, 500)
        ...
        # Find safe speed for this segment
         while speed >= 1:
            ...
            if rao_key not in self.rao_lib.RAO_dict:
                self.rao_lib.add_rao(rao_key[0],rao_key[1],rao_key[2],rao_key[3],model)
            ...
            integration = np.sqrt(np.trapz(response_spectra,x=rao.freq))
            if(speed !=0):
    ...
    def calculate_new_position(self, current_pos,speed,time_elapsed):
        ...
        position=geod.Direct(current_pos.lat,current_pos.long, self.heading, dist_traveled)
    ...
    def is_at_waypoint(self, waypoint, time_interval):
        ...
        '''
        g=geod.Inverse(self.position.lat, self.position.long, waypoint.lat,waypoint.long)
    ...
    def calc_time_diff(self, current_time, goal_time,verbose=False):
        ...
        if(verbose):
         if current_time > goal_time:
          print(f"{self.name} arrived {hrs_difference} hours {min_difference} minutes and {sec_difference} seconds late")
         elif current_time <goal_time:
          print(f"{self.name} arrived {hrs_difference} hours {min_difference} minutes and {sec_difference} seconds early")
         else:
    ...
    def add_opt_mission_to_db(self,reason,current_time):
       ...
       for order, dest in enumerate(self.opt_mission.destinations):
           db_manager.insert_mission_waypoint(self.mission_id, dest.id, self.replan_count, order, dest.arrival_time,dest.estimated_arrival_time)
       db_manager.insert_replan(self.replan_count,self.vessel_id,self.mission_id,current_time,self.position.lat,self.position.long,self.opt_mission.total_priority,len(self.opt_mission.destinations),reason)
...
class fuelCalc(baseCalc):
    ...
    def provideValues(self):
        return ("fuel", {"vessel_id": self.vessel.vessel_id, "name": self.vessel.name,"fuel_weight": self.vessel.fuel_weight})
...
class resistanceCalc(baseCalc):
    ...
    def tableOutputs(self):
        return ("resistance_model",{"vessel_id": "integer", "name": "text","speed": "real", "resistance_in_kn": "real","wake_fraction": "real","thrust_deduction": "real"})
    
    def provideValues(self):
        return ("resistance_model", {"vessel_id": self.vessel.vessel_id, "name": self.vessel.name,"speed": self.vessel.speed, "resistance_in_kn": self.vessel.resistance_in_kn,"wake_fraction": self.vessel.wake_fraction,"thrust_deduction": self.vessel.thrust_deduction})
...
class propulsionCalc(baseCalc):
    ...
    def tableOutputs(self):
        return ("propulsion_model",{"vessel_id": "integer", "name": "text","speed": "real", "resistance_in_kn": "real","wake_fraction": "real","thrust_deduction": "real","eta_O": "real","rpm": "real", "p_deliv": "real"})
    
    def provideValues(self):
        return ("propulsion_model", {"vessel_id": self.vessel.vessel_id, "name": self.vessel.name,"speed": self.vessel.speed, "resistance_in_kn": self.vessel.resistance_in_kn,"wake_fraction": self.vessel.wake_fraction,"thrust_deduction": self.vessel.thrust_deduction,"eta_O": self.vessel.eta_O,"rpm": self.vessel.RPM, "p_deliv": self.vessel.P_deliv})    
...
class engineCalc(baseCalc):
    ...
    def tableOutputs(self):
        return ("engine_model", {"vessel_id": "integer" ,"name": "text", "rpm": "real","p_deliv": "real","fuel_burned": "real"})
    
    def provideValues(self):
        return ("engine_model", {"vessel_id": self.vessel.vessel_id, "name": self.vessel.name,"rpm": self.vessel.RPM, "p_deliv": self.vessel.P_deliv,"fuel_burned": self.vessel.fuel_burned})

class vesselFuel(vesselBase):
    '''
    sub class of vesselBase adds fuel calculations
    '''
    def __init__(self,name,sim_name,start,end,mission,fuel_weight,weather_input_dir,fc_input_dir=None,model_path=None,speed=0,heading=0,speed_options=[]):
        '''
        ...
        :param speed_options: list of floats, all different speed settings for the vessel
        '''
        super().__init__(name,sim_name,start,end,mission,weather_input_dir,fc_input_dir,model_path,speed,heading,fuel_weight,speed_options)
        ...
        self.fuel_vals = [[256.8], [230.8], [217.3], [212.1], [208.9], [208.3], [212.2], [222.4]]
        engine1 = RPM_EngineModel( self.RPM_vals, self.power_vals, self.fuel_vals, 10)
        self.propSim1=propulsionSimulationBase(self.res_model,self.propeller,engine1)
    ...
    def calc_fuel_burned(self,speed,time_interval):
        ...
        :param time_interval: float, amount of time that passed in hours
        '''
        self.fuel_burned,sfc,(self.eta_O,self.P_deliv,self.RPM),(self.resistance_in_kn,self.wake_fraction,self.thrust_deduction)=self.propSim1.runMachinery(speed,self,time_interval)
        
    def getHydrostaticProperties(self):
        '''
        returns the set hydrostatic properties
        '''
        return {'Length': 45.0, 'Beam': 7.0, 'WettedSurface': 100.0, 'Draft': 3.0, 'Volume': 472.5, 'WindageArea': 0.0, 'AirDragCoefficient': 0.0}
    ...
    def get_fuel_time(self,current_time,time_interval,fuel_burned):
        ...
        self.position.lat=pos[0]
        self.position.long=pos[1]
        logger.info(f"{self.name} ran out of fuel at {current_time} and position ({self.position.lat}, {self.position.long})\n")
...
class Simulation:
    ...
    def __init__(self,vessels, start_time, end_time,display_map=True):
        ...
        self.colors=['lightcoral','orangered','gold','honeydew','lawngreen','cyan','violet','deeppink','lightpink']
        counter=0
        
        for vessel in self.vessels:
         color=self.colors[counter%len(self.colors)]
         self.sc.append(self.ax.scatter(vessel.position.long, vessel.position.lat, marker='^', color=color, s=100, transform=ccrs.PlateCarree(),label=vessel.name))
         ...
         for destination in vessel.full_mission.destinations:
           ...
           if count==0:
            self.ax.scatter(waypoint.long, waypoint.lat, marker='.', color=color, s=100, transform=ccrs.PlateCarree(), label=f"{vessel.name}'s waypoint")
            count=1
           else:
               self.ax.scatter(waypoint.long, waypoint.lat, marker='.', color=color, s=100, transform=ccrs.PlateCarree())
         self.ax.scatter(vessel.end.waypoint.long, vessel.end.waypoint.lat, marker='.', color=color, s=100, transform=ccrs.PlateCarree()) 
         self.ax.legend()
         vessel.makeTables()
         counter+=1 
        plt.ion()
         ...
         if(vessel.active and current_time>= vessel.start.arrival_time):
          if not vessel.navigate(current_time, time_step):
           #moves vessel to final waypoint
            lastwp=vessel.opt_mission.destinations[len(vessel.opt_mission.destinations)-1].waypoint
            self.sc[i].set_offsets((lastwp.long,lastwp.lat))
            db_manager.insert_log(vessel.vessel_id, vessel.mission_id, current_time.strftime('%Y-%m-%d %H:%M:%S'), vessel.position.lat, vessel.position.long, vessel.speed, 'Good', vessel.fuel_weight, 0, 0)
            vessel.active=False
          else:
             #updates marker position/rotation to match vessel
             if self.display_map:
              self.sc[i].set_offsets((vessel.position.long, vessel.position.lat))
...
# Example usage
 ...
 destinations1=[
                    ...
                    Destination(Waypoint(42,-138),datetime(2024,12,22,15,0),77),Destination(Waypoint(40, -137), datetime(2024, 12, 21, 5, 0), 7),
                    Destination(Waypoint(40.5, -138), datetime(2024,12, 22, 16, 0), 9),Destination(Waypoint(39,-136.5),datetime(2024,12,22,15,0),6)]
 destinations2=[
                    ...
                    Destination(Waypoint(40,-139),datetime(2024,12,21,17,0),7), Destination(Waypoint(42, -137), datetime(2024, 12, 23, 5, 0), 17),
 ...
 destinations3=[
                    ...
                    Destination(Waypoint(35,159),datetime(2024,12,22,17,0),17),Destination(Waypoint(35, 157), datetime(2024, 12, 21, 15, 0), 87),
 ...
 destinations4=[
                    ...
                    Destination(Waypoint(35.5,155),datetime(2024,12,22,17,0),17), Destination(Waypoint(35.5, 152), datetime(2024, 12, 21, 20, 0), 19),
 ...
 dests_small=[Destination(Waypoint(38.5, -137), datetime(2024, 12, 20, 5, 0), 7),
                    Destination(Waypoint(38.5, -138), datetime(2024,12, 26, 6, 0), 9)]
 ...
 vessel_fc_test1=vesselFuel("vessel1","sim1",Destination(Waypoint(40,-139),datetime(2024, 12, 20, 0, 0),0),Destination(Waypoint(40,-139),datetime(2024, 12, 24, 7, 0),0), fc_test_mission1, 50,weath_input_dir_lat40_lon_neg_140,model_path=model_path,speed=20,speed_options=[15,20,25])
 vessel_fc_test2=vesselFuel("vessel2","sim1", Destination(Waypoint(40,-139),datetime(2024, 12, 20, 0, 0),0),Destination(Waypoint(40,-139),datetime(2024, 12, 24, 7, 0),0), fc_test_mission2, 50,weath_input_dir_lat40_lon_neg_140,model_path=model_path
                           ,speed=20,speed_options=[15,20,25])
 vessel_fc_test3=vesselFuel("vessel3","sim1", Destination(Waypoint(36,155),datetime(2024, 12, 20, 0, 0),0),Destination(Waypoint(36,155),datetime(2024, 12, 24, 7, 0),0), fc_test_mission3, 50,weath_input_dir_lat35_lon155,model_path=model_path
                           ,speed=20,speed_options=[15,20,25])
 vessel_fc_test4=vesselFuel("vessel4","sim1", Destination(Waypoint(35,155),datetime(2024, 12, 20, 0, 0),0),Destination(Waypoint(35,155),datetime(2024, 12, 24, 7, 0),0), fc_test_mission4, 50,weath_input_dir_lat35_lon155,fc_input_dir=fc_input_dir_lat35_lon155
                           ,speed=20,speed_options=[15,20,25])
 vessel_small=vesselFuel("vessel1","sim1",Destination(Waypoint(40,-139),datetime(2024, 12, 20, 0, 0),0),Destination(Waypoint(40,-139),datetime(2024, 12, 24, 7, 0),0), mission_small, 50,weath_input_dir_lat40_lon_neg_140,model_path=model_path,speed=20,speed_options=[15,20,25])
 simulation = Simulation([vessel_fc_test1,vessel_fc_test2,vessel_fc_test3,vessel_fc_test4], datetime(2024, 12, 20,0),datetime(2024, 12,31, 0, 0))
