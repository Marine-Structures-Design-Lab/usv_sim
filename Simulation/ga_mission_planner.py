...
class Individual:
    ...
    def __gt__(self, other):
        ...
        """
        if self.violation_sum == 0 and other.violation_sum != 0:
            return True
        elif other.violation_sum == 0 and self.violation_sum != 0:
            return False
        elif self.violation_sum != 0 and other.violation_sum != 0:
            if self.violation_sum < other.violation_sum:
                return True
            elif self.violation_sum > other.violation_sum:
                return False
            else:
...
class Optimizer(object):
    ...
    def calculate_fuel(self, mission, edit_estimates=False):
        ...
        for i in range(0, len(mission.destinations) - 1):
            time_left = (
                mission.destinations[i].arrival_time - timestamp
            ).total_seconds() / 3600
            ...
            g = geod.Inverse(
                mission.destinations[i].waypoint.lat,
                mission.destinations[i].waypoint.long,
                mission.destinations[i + 1].waypoint.lat,
                mission.destinations[i + 1].waypoint.long,
            )
            distance = g["s12"] / 1852
            self.prob.vessel.heading = g["azi1"]
            old_speed = self.prob.vessel.speed
            self.prob.vessel.speed = mission.speeds[i]
            speed = self.prob.vessel.adjust_speed(
                mission.destinations[i].waypoint,
                distance,
                self.prob.start_time,
                timestamp,
            )
        ...
        if avg_speed == 0:
            return 0
        total_fuel = self.prob.vessel.propSim1.runMachinery(
            avg_speed, self.prob.vessel, total_time
        )[0]
    ...
    def make_vessel_individual(self, chrom):
        ...
        norm_fuel_left = (
            self.prob.vessel.fuel_weight - fuel_used
        ) / self.prob.vessel.fuel_weight
        # if vessel doesn't have enough fuel to complete mision
        if fuel_used > self.prob.vessel.fuel_weight:
            violation_sum = (
                fuel_used - self.prob.vessel.fuel_weight
            ) / self.prob.vessel.fuel_weight

        ind = Individual(
            chrom, 1, violation_sum, norm_priority, norm_fuel_left, fuel_used
        )
        return ind
    ...
    def crossover(self, parent1, parent2):
        ...
        if len(parent1[0]) < len(parent2[0]):
            shortest_len = len(parent1[0])
        else:
            shortest_len = len(parent2[0])
        if shortest_len <= 2:
        ...
        if (
            chrom1[0][0] != 0
            or chrom1[0][len(chrom1[0]) - 1] != len(self.prob.waypoints) - 1
        ):
            logger.error("chrom1 start/end error during crossover")
        if (
            chrom2[0][0] != 0
            or chrom2[0][len(chrom2[0]) - 1] != len(self.prob.waypoints) - 1
        ):
    ...
    def partial_reverse_waypoints(self, chrom):
        ...
        if (
            chrom[0][0] != 0
            or chrom[0][len(chrom[0]) - 1] != len(self.prob.waypoints) - 1
        ):
    ...
    def local_search_speed(self, individual, positions_to_try=5):
        ...
        positions = self.rng.sample(
            range(num_speeds), k=min(positions_to_try, num_speeds)
        )
    ...
    def find_percent_ind(self, pop, ind):
        ...
        for i in pop:
            if (
                i.chromosome[0] == ind.chromosome[0]
                and i.chromosome[1] == ind.chromosome[1]
            ):
                count += 1
    ...
    def run(self, max_generations=50):
        ...
        if self.Nwp < 1:
            return (
                [self.prob.start, self.prob.end],
                [self.prob.speeds[0], self.prob.speeds[0]],
                self.prob.vessel.fuel_weight,
                0,
            )
        ...
        while self.current_generation < max_generations:
            ...
            if self.current_generation % 5 == 0:
                ...
                percents_bests.append(
                    (
                        self.find_percent_ind(pop, elite[self.Nelite - 1]),
            ...
            while len(selection) > 1:
                ...
                if self.rng.random() < self.PCross:
                    ...
                    children = self.crossover(
                        children[0].chromosome, children[1].chromosome
                    )
                    mutations = [
            ...
            mean_scores_no_violation.append(
                self.calc_mean_score_no_violations(next_gen)
            )
        ...
        if elite[self.Nelite - 1].violation_sum == 0:
            ...
            opt_dests = [
                self.prob.waypoints[elite[self.Nelite - 1].chromosome[0][0]],
                self.prob.waypoints[elite[self.Nelite - 1].chromosome[0][-1]],
            ]
            elite[self.Nelite - 1].fitness = 0
        # opt_dests.append(self.prob.end)
        fuel = self.calculate_fuel(
            Mission(opt_dests, elite[self.Nelite - 1].chromosome[1]), True
        )
        ...
        return (
            opt_dests,
            elite[self.Nelite - 1].chromosome[1],
            fuel_left,
            elite[self.Nelite - 1].fitness,
        )
