...
def test_get_file_list_returns_empty_for_nonexistent_directory(tmp_path, caplog):
    fake_dir = tmp_path / "nonexistent" #! doesn't create directory unless calles fake_dir.mkdir()
...
def test_read_initial_data_reads_valid_file(reset_unit, monkeypatch, caplog):
    ...
    dummy_ds.variables = {
        ...
        'lon': np.array([30, 40]),
        'perpw': MagicMock(**{
            '__getitem__.return_value': np.array([[1.0, 2.0]]),
            'units': 'm'
        })
    }
...
def test_read_initial_data_corrupt_file(reset_unit, monkeypatch):
    ...
    monkeypatch.setattr("netCDF4.Dataset", lambda _: (_ for _ in ()).throw(OSError("Corrupt file")))
