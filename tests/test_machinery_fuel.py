...
def test_engine(fuel_vessel):
    ...
    print(
        f"RPM Engine Model RPM=300(too large) {engine1.getSFC(300, 90, fuel_vessel)} "
    )
    assert engine1.getSFC(300, 90, fuel_vessel) == pytest.approx(222.4, 0.001)
    print(
        f"RPM Engine Model, RPM=100(power too large): {engine1.getSFC(100, 92, fuel_vessel)}"
    )
    assert engine1.getSFC(100, 92, fuel_vessel) == pytest.approx(230.8, 0.001)
    engine2 = RPM_Power_EngineModel(RPM2, power2, fuel2, 10)
    print(
        f"RPM Power Engine Model, RPM=100,Power 96 kW: {engine2.getSFC(100, 96, fuel_vessel)}"
    )
    assert engine2.getSFC(100, 96, fuel_vessel) == pytest.approx(224.5273, 0.001)
    print(
        f"RPM Power Engine Model, RPM=170, Power 500 kW: {engine2.getSFC(170, 500, fuel_vessel)}"
    )
    assert engine2.getSFC(170, 500, fuel_vessel) == pytest.approx(203.8945, 0.001)
    print(
        f"RPM Power Engine Model, RPM=170, Power 700(too big)  kW: {engine2.getSFC(170, 700, fuel_vessel)}"
    )
    assert engine2.getSFC(170, 700, fuel_vessel) == pytest.approx(201.1986, 0.001)
    print(
        f"RPM Power Engine Model, RPM=7(too small), Power 50 kW: {engine2.getSFC(7, 50, fuel_vessel)}"
    )
    assert engine2.getSFC(7, 50, fuel_vessel) == pytest.approx(242.8202, 0.001)
    print(
        f"RPM Power Engine Model, RPM=7(too small), Power 500kW(too big) kW: {engine2.getSFC(7, 5000, fuel_vessel)}"
    )
...
def test_propeller(fuel_vessel):
    ...
    eta, delivered_power, RPM = propeller.estimatePropulsion(
        25.0, Rt, wt, t, fuel_vessel, res_model.rho
    )
    ...
    eta, delivered_power, RPM = propeller.estimatePropulsion(
        15.0, Rt, wt, t, fuel_vessel, res_model.rho
    )
    ...
    eta, delivered_power, RPM = propeller.estimatePropulsion(
        10.0, Rt, wt, t, fuel_vessel, res_model.rho
    )
...
def test_resistance(fuel_vessel):
    ...
    assert drag_25kts == (
        pytest.approx(45.1658, 0.001),
        pytest.approx(0.10375, 0.001),
        pytest.approx(0.1649, 0.001),
    )
    print("Drag at 20 kts: " + str(drag_20kts))
    assert drag_20kts == (
        pytest.approx(29.173497, 0.001),
        pytest.approx(0.10375, 0.001),
        pytest.approx(0.1649, 0.001),
    )
    print("Drag at 10 kts: " + str(drag_10kts))
    assert drag_10kts == (
        pytest.approx(7.52097, 0.001),
        pytest.approx(0.10375, 0.001),
        pytest.approx(0.1649, 0.001),
    )
