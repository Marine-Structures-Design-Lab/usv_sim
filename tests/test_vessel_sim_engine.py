...
import pytest
from Simulation.vessel_sim_engine import  Waypoint, Mission,Destination, vesselBase, vesselFuel
from geographiclib.geodesic import Geodesic
from Simulation.machinery_fuel import propulsionSimulationBase,PropellerPropulsionModel,RPM_EngineModel,NPL_ResistanceModel
geod = Geodesic.WGS84
...
def base_vessel():
    ...
    vessel = vesselBase("base_vessel","test_sim",Destination(Waypoint(3, 54.5),datetime(2024, 12, 7, 20, 0),0),Destination(Waypoint(3, 54.5),datetime(2024, 12, 9, 7, 0),0),mission,ocean_path,model_path=model_path,speed=20)
...
def fuel_vessel():
    ...
    mission = Mission(destinations)
    vessel = vesselFuel("fuel_vessel","test_sim",Destination(Waypoint(3, 54.5),datetime(2024, 12, 7, 20, 0),0),Destination(Waypoint(3, 54.5),datetime(2024, 12, 9, 7, 0),0),mission,100,ocean_path,model_path=model_path,speed=20)
...
def test_navigate_heading_position(fuel_vessel):
    ...
    heading1=geod.Inverse(starting_position.waypoint.lat, starting_position.waypoint.long, fuel_vessel[0][0].waypoint.lat, fuel_vessel[0][0].waypoint.long)['azi1']
    #heading between waypoint1 and waypoint2
    heading2=geod.Inverse( fuel_vessel[0][0].waypoint.lat, fuel_vessel[0][0].waypoint.long, fuel_vessel[0][1].waypoint.lat, fuel_vessel[0][1].waypoint.long)['azi1']
    #heading between waypoint2 and waypoint3
    heading3=geod.Inverse(fuel_vessel[0][1].waypoint.lat, fuel_vessel[0][1].waypoint.long, fuel_vessel[0][2].waypoint.lat, fuel_vessel[0][2].waypoint.long)['azi1']
    fuel_vessel[2].fuel_weight=10000
    fuel_vessel[2].navigate(datetime(2024, 12, 7, 20, 0),1)
    distance=fuel_vessel[2].speed*1852*1
    g1=geod.Direct(starting_position.waypoint.lat, starting_position.waypoint.long, heading1, distance)
    lat=g1['lat2']
    long=g1['lon2']
    ...
    g=geod.Direct(starting_position.waypoint.lat, starting_position.waypoint.long, heading1, distance)
    lat=g['lat2']
    long=g['lon2']
...
def test_adjust_speed(fuel_vessel):
    ...
    fuel_vessel[2].speed=fuel_vessel[2].adjust_speed(Waypoint(-8,53),200,datetime(2024,12, 6, 7, 0),datetime(2024,12, 6, 7, 0))
    assert fuel_vessel[2].speed==20
    fuel_vessel[2].heading=0
    fuel_vessel[2].speed=fuel_vessel[2].adjust_speed(Waypoint(0,54),100,datetime(2024,12, 12, 14, 0),datetime(2024,12, 12, 14, 0))
    assert fuel_vessel[2].speed==20
    fuel_vessel[2].speed=20
    fuel_vessel[2].heading=250
    fuel_vessel[2].speed=fuel_vessel[2].adjust_speed(Waypoint(3,58),200,datetime(2024,12, 7, 7, 0),datetime(2024,12, 7, 7, 0))
    assert fuel_vessel[2].speed==20
    
def test_calculate_position(base_vessel): 
    base_vessel[2].calculate_new_position(base_vessel[2].position,base_vessel[2].speed,0)
    ...
    position=geod.Direct(base_vessel[2].position.lat,base_vessel[2].position.long, base_vessel[2].heading, dist_traveled)
    assert base_vessel[2].calculate_new_position(base_vessel[2].position,base_vessel[2].speed,0.5)==(position['lat2'], position['lon2'])
    dist_traveled=base_vessel[2].speed*2*1852
    position=geod.Direct(base_vessel[2].position.lat,base_vessel[2].position.long, base_vessel[2].heading, dist_traveled)
    assert base_vessel[2].calculate_new_position(base_vessel[2].position,base_vessel[2].speed,2)==(position['lat2'], position['lon2'])
...
def test_fuel_calc(fuel_vessel,fuel_model):
    ...
    fuel_burned,sfc,(eta_O,P_deliv,RPM),(resistance_in_kn,wake_fraction,thrust_deduction)=fuel_model.runMachinery(fuel_vessel[2].speed,fuel_vessel[2],time_interval)
