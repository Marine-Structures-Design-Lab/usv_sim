...
@pytest.fixture 
def ga():
 destinations = [Destination(Waypoint(40, -137), datetime(2024, 12, 21, 5, 0), 7),
 ...
 vessel=vesselFuel("vessel2", "testsim2", Destination(Waypoint(40,-139),datetime(2024, 12, 20, 0, 0),0),Destination(Waypoint(40,-139),datetime(2024, 12, 24, 7, 0),0), mission, 50,weath_input_dir_lat40_lon_neg_140,model_path=model_path
...
def test_calculate_fuel(ga):
 ...
 assert ga.calculate_fuel(Mission(ga.prob.waypoints,speeds))==pytest.approx(29.9333,0.001)
 speeds=[5]*len(ga.prob.waypoints)
 assert ga.calculate_fuel(Mission(ga.prob.waypoints,speeds))==pytest.approx(2.463,0.001)
 ...
 assert ga.calculate_fuel(Mission( [Destination(Waypoint(40,-139), datetime(2024, 12, 20, 0, 0),9)],[10]))==0
def test_make_vessel_individual(ga):
 chrom=[[0,1,2,len(ga.prob.waypoints)-1],[5,10,5,10]]
 ind=ga.make_vessel_individual(chrom)
 mission=Mission([ga.prob.start,ga.prob.waypoints[1],ga.prob.waypoints[2],ga.prob.end],[5,10,5,10])
 ...
 ind=ga.make_vessel_individual(chrom)
 mission=Mission([ga.prob.start,ga.prob.waypoints[6],ga.prob.end],[20,10,15,20])
...
def test_individual_comparison(ga):
 ...
 ind5=ga.make_vessel_individual([[0,3,6,5,4,1,2,7],[20,25,20,10,15,20,20,20]])
 ind6=ga.make_vessel_individual([[0,3,6,5,4,1,2,7], [25,25,25,25,25,25,25,20]])
 ind7=ga.make_vessel_individual([[6,8],[15,20]])
 ind8=ga.make_vessel_individual([[0,6],[15,15]])
 ind9=ga.make_vessel_individual([[4,6],[20,20]])
...
destinations = [Destination(Waypoint(40, -137), datetime(2024, 12, 21, 5, 0), 7),
                    ...
                    Destination(Waypoint(40,-139),datetime(2024,12,21,17,0),7)]
mission=Mission(destinations)
vessel=vesselFuel("vessel2","test_sim2", Destination(Waypoint(40,-139),datetime(2024, 12, 20, 0, 0),0),Destination(Waypoint(40,-139),datetime(2024, 12, 24, 7, 0),0), mission, 50,weath_input_dir_lat40_lon_neg_140,model_path=model_path
                           ,speed=20,speed_options=[15,20,25])
problem=VesselProblem(vessel,datetime(2024, 12, 20, 0, 0))
opt=Optimizer(problem,94140,20)
test_run(opt)