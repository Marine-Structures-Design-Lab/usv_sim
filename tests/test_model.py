...
def create_sample_NOAA_grid():
    ...
    time = fout.createVariable('time', np.float32,
                               ('time',), compression='zlib')
...
def create_sample_n_var_grid():
    ...
    data = np.zeros((5, 5, 5))
    for i in range(n):
        vars.append(fout.createVariable(
...
def test_mission_diff_length_inputs():
    ...
    with pytest.raises(ValueError):
        model.weather_mission(
            [datetime(2018, 1, 1, hour=2)], [(0, 0), (1, 1)])
...
def test_NOAA_interpolation():
    ...
    model.load_data('Ocean/NOAA_test_suite_grid.nc')
    dates = [datetime(
        2018, 1, 1) + timedelta(hours=(random() * 9)) for _ in range(10)]
    ...
    assert names == ['sig_wave_height', 'wave_direction',
...
def test_n_variable_file():
    ...
    var_names = [u for u in unit_names if u !=
                 'time' and u != 'lat' and u != 'lon']
