...
for m in months:
    ...
    files = ['multi_1.{g}.{p}.{d}.grb2'.format(
        g=grid_id, p=p_id, d=m) for p_id in ['hs', 'tp', 'dp', 'wind']]
    for var, f in enumerate(files):
        ...
        if var == 0:  # significant wave height
            ...
            u_grib = grib_file.select(
                shortName='u', validDate=dates_this_month)
            v_grib = grib_file.select(
                shortName='v', validDate=dates_this_month)
