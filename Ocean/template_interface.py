...
class OceanMod:
    ...
    def load_data(self, filename):
        ...
        self.time = num2date(fin.variables['time'][:],
        ...
        def to_float(d): return calendar.timegm(d.timetuple())
        time_dim = list(map(to_float, self.time))
        lat_dim = self.latitude
        lon_dim = self.longitude
        interpolation_vars = [v for v in vars if v !=
                              'lat' and v != 'lon' and v != 'time']
    ...
    def weather_mission(self, dates, coordinates):
        ...
        try:
            ...
            raise ValueError('one or more input values is outside'
                             'of the model domain') from e
    ...
    def plot_grid(self, timestamp, var_type):
        ...
        if var_type == 'pressure' and 'surface_pressure' not in \
                self.interpolators.keys():
            raise ValueError('"pressure" is not one of the variables '
        ...
        if var_type == 'wind':
            if timestamp in self.time:
                ...
                ws = ws_left * ((self.time[right] - timestamp) / delta) \
                    + ws_right * ((timestamp - self.time[left]) / delta)
                wd = wd_left * ((self.time[right] - timestamp) / delta) \
                    + wd_right * ((timestamp - self.time[left]) / delta)
            wd = np.radians((((450 - wd) % 360) + 180) % 360)
            U = ws * np.cos(wd)
            V = ws * np.sin(wd)
            Q = plt.quiver(X[::latstep, ::lonstep], Y[::latstep, ::lonstep],
                           U[::latstep, ::lonstep], V[::latstep, ::lonstep],
                           color='g', pivot='middle')
            qk = plt.quiverkey(Q, 0.95, 0.95, 10, r'$10 \frac{m}{s}$',
            ...
            if timestamp in self.time:
                ...
                delta = self.time[right] - self.time[left]
                H = Z_left * ((self.time[right] - timestamp) / delta) + \
                    Z_right * ((timestamp - self.time[left]) / delta)
                ...
                T = T_left * ((self.time[right] - timestamp) / delta) + \
                    T_right * ((timestamp - self.time[left]) / delta)
            ...
            U = np.cos(wave_dir)
            V = np.sin(wave_dir)
            plt.quiver(X[::latstep, ::lonstep], Y[::latstep, ::lonstep],
                       U[::latstep, ::lonstep], V[::latstep, ::lonstep],
                       pivot='middle', zorder=2)
            plt.title('Significant Wave Height and Wave Direction at \nTime {}'
                      .format(timestamp))
            plt.grid(zorder=1)
            plt.savefig('wave_height_direction_{}.png'.format(
                timestamp.isoformat()))
            fig.clear()
            ...
            if timestamp in self.time:
                ...
                delta = self.time[right] - self.time[left]
                P = P_left * ((self.time[right] - timestamp) / delta) + \
                    P_right * ((timestamp - self.time[left]) / delta)
