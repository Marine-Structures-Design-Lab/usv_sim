...
hs = fout.createVariable('sig_wave_height', np.float32,
                        ('time', 'lat', 'lon'), compression='zlib')
hs.units = 'meters'
tp = fout.createVariable('wave_period', np.float32,
                        ('time', 'lat', 'lon'), compression='zlib')
tp.units = 'seconds'
dp = fout.createVariable('wave_direction', np.float32,
                        ('time', 'lat', 'lon'), compression='zlib')
dp.units = 'degrees true'
ws = fout.createVariable('wind_speed', np.float32,
                        ('time', 'lat', 'lon'), compression='zlib')
wd = fout.createVariable('wind_direction', np.float32,
                        ('time', 'lat', 'lon'), compression='zlib')
ws.units = 'meters per second'
wd.units = 'degrees true'


ap = fout.createVariable('surface_pressure', np.float32,
                        ('time', 'lat', 'lon'), compression='zlib')
ap.units = 'pascal'
...
_, sample_lats, sample_lons = extract_grid(lat_min, lat_max, lon_min, lon_max, ds.readline())
...
try:
    ...
    fin_hs = sorted(ds.select(name='Significant height of combined wind waves and swell', validDate = dates), key=lambda x: x.validDate)
...
try:
    ...
    fin_tp = sorted(ds.select(name='Peak wave period', validDate = dates), key=lambda x: x.validDate)
    for grid in (fin_tp):
        time_index = int((grid.validDate - start)/delta)
...
try:
    ...
    fin_dp = sorted(ds.select(name='Mean wave direction', validDate = dates), key=lambda x: x.validDate)
    for grid in (fin_dp):
        time_index = int((grid.validDate - start)/delta)
        data, _, _ = extract_grid(lat_min, lat_max, lon_min, lon_max, grid)
...
try:
    ...
    fin_ap = sorted(ds.select(name='Surface pressure', validDate = dates), key=lambda x: x.validDate)
    for grid in (fin_ap):
        time_index = int((grid.validDate - start)/delta)
        data, _, _ = extract_grid(lat_min, lat_max, lon_min, lon_max, grid)
        ap_data = data.data
...
try:
    ...
    fin_u = sorted(ds.select(name='10 metre U wind component', validDate = dates), key=lambda x: x.validDate)
    fin_v = sorted(ds.select(name='10 metre V wind component', validDate = dates), key=lambda x: x.validDate)
