    ...
    for level_idx in range(level):
        for lat_idx in range(lat_size):
            for lon_idx in range(lon_size):
                ...
                switches = sum(
                    angle_diff(angles[t], angles[t + 1]) > threshold
                    for t in range(len(angles) - 1)
                )
                switches_count[lat_idx, lon_idx] = switches

        avg_switches = np.mean(switches_count)
        print(
            f"Average number of >{threshold}° switches per grid point in level {level_idx}: {avg_switches:.2f}/ {time_size}"
        )

        total_switches = np.sum(switches_count)
        print(
            f"Total number of >{threshold}° switches for all in level {level_idx}: {total_switches}/ {lat_size*lon_size * time_size}"
        )
