    ...
    def timeSeries(self, times, phases=None):
        ...
        if phases is None and self.phases is None:
            ...
            if len(phases) != len(self.AmpComponents):
                raise Exception('Phase/Amp Components of different length' +
                                ' in BaseSpectra')
...
class JONSWAP(BaseSpectra):
    def __init__(self, name, H13, Tp, Gamma=3.3, wMin=0.001, wMax=5,
                 nComponents=500):
        '''
        ...
        for index, Omega in enumerate(self.Omegas):
            ...
            self.Spectrum[index] = (320. * H13**(2.0)/Tp**(4.0) * Omega**(-5.0)
                                    * exp(-1950/Tp**(4.0) * Omega**(-4.0))
                                    * Gamma**(A))

            # Calculate Amplitude coefficient - 5.129 in Journee
            self.AmpComponents[index] = (sqrt(2.0) * (self.Spectrum[index] *
                                                      deltaOmega)**(0.5))
...
# self, time, numcomponents, phases=None
    def timeSeries():
        ...
        for i in range(elem):
            ct = cos(np.astype(
                np_array_nc[i])*x-(w*np.astype(np_array_time[i])) +
...
class Bretschneider(BaseSpectra):
    def __init__(self, name, H13, T1, wMin=0.01, wMax=3.0, numComponents=500):
        ...
        for index, Omega in enumerate(self.Omegas):
            ...
            self.Spectrum[index] = (
                (173 * H13**(2)) / T1**(4)) * Omega**(-5) * exp((-692/T1**(4))
                                                                * Omega**(-4))
