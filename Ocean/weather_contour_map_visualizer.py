...
import numpy as np
from Ocean.weather_utils import filelist_editor, get_file_list, read_grid_info, read_variable_data, update_fcst_time
import logging
...
def update_frame(frame, file_list, mesh, title, VARIABLE_NAME):
    ...
    logger.debug(f"Frame {frame}: Forecast time = {fcst_datetime}, Issued = {issue_datetime}")

    if issue_datetime == fcst_datetime:
        title.set_text(f"{VARIABLE_NAME.upper()} on {fcst_datetime.strftime('%Y%m%d at %H:%M')}")
    else:
        title.set_text(
